$(function() {
 ...
 // 	value_filter_siswa() 	
	...
	var table4 = $('#advanced-usage').DataTable({
		"ajax": {
			"url": "<?= base_url()?>absen/keluar/ajax_data/"+id,
			"data": null,
			"type": 'POST'
		},
		"columns": [
			...
			{ "data": "sisw_id", render(data, type, full, meta)
				{
					...
					{
						return full.sisw_status_keluar
					}
				}
			}
		],
	...
	$('#advanced-usage tbody').on('click', '.hadir-button', function(ev) {
		...
		window.apiClient.absenKeluar.hadir(content, kksi_kkdj_id)
		.done(function(data) {
	...
	$('#advanced-usage tbody').on('click', '.sakit-button', function(ev) {
		...
		window.apiClient.absenKeluar.getDataSiswa(siswa)
		.done(function(data)
		{
	...
	$('#form').submit(function(ev) 
	{
		...
		if(status == 'Izin')
		{
			window.apiClient.absenKeluar.izin(content, kksi_kkdj_id, keterangan)
			.done(function(data) {
				$("#advanced-usage").dataTable().fnDestroy()
		...
		{
			window.apiClient.absenKeluar.sakit(content, kksi_kkdj_id, keterangan)
			.done(function(data) {
				$("#advanced-usage").dataTable().fnDestroy()
		...
		{
			window.apiClient.absenKeluar.alpa(content, kksi_kkdj_id, keterangan)
			.done(function(data) {
				$("#advanced-usage").dataTable().fnDestroy()
	...
	scanner.addListener('scan', function (content) 
	{
		...
		ajax = window.apiClient.absenKeluar.hadir(content, kksi_kkdj_id)
			.done(function(data) {
				$.message('Absen',data.sisw_nama,'success')
	...
	{
		// location.href = '<?= base_url() ?>absen/keluar/siswa/'+id
		$("#advanced-usage").dataTable().fnDestroy()
		dynamic()
	})
